...
namespace ShittyOBJLoader
{
	...
	inline DirectX::XMFLOAT3 CalculateTangent(MeshVertex vertex1, MeshVertex vertex2, MeshVertex vertex3)
	{
	...
	inline Mesh* Load (std::string path, ID3D11Device* device)
	{
		...
		if (fs.is_open())
		{
			...
			{
				tangent = CalculateTangent(vertexes[indices[i * 3 + 0]], vertexes[indices[i * 3 + 1]], vertexes[indices[i * 3 + 2]]);
