...
{
public:
	Spoon(const char* name, Terrain* terrain, DX11Handler& dx11, AssimpHandler::AssimpData model, DirectX::XMVECTOR position, DirectX::XMVECTOR rotation, /* SoundHandler* soundeffect, */ Gamemanager* gamemanager); // tabort soundeffect
	...
	void PlaySoundEffect() override { /*weaponSound->PlaySound("Swoosh", 0.1f);*/ gamemanager->GetSoundeffectHandler()->PlaySound("Swooshsound", gamemanager->GetCurrentSoundVolume()); }

	void Use() override { this->used++; };
	int CheckUsage() override { return this->used; }
private:	
	//int movementspeeds = 3;
	//SoundHandler* weaponSound;	//tabrot
